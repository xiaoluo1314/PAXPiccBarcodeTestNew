...
public class CenterPositon extends JDialog {
	...
	public CenterPositon(JFrame jDialog) {
		super(jDialog, true);
		setTitle("中心点");
		try {
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				int i = JOptionPane.showConfirmDialog(CenterPositon.this, "中心点是否确定？如没有则执行会报错，请慎重关闭！","关闭",JOptionPane.YES_NO_OPTION);
...
//		xField.setText("1");
		...
		xComboBox.setPreferredSize(new Dimension(60, xComboBox.getPreferredSize().height));
		xComboBox.setFocusable(false);
		...
		finded.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				new Thread(new Runnable() {
					public void run() {
						int i = JOptionPane.showConfirmDialog(CenterPositon.this, "是否进行中心点确定","中心点确定",JOptionPane.YES_NO_OPTION);
						if (i !=JOptionPane.YES_OPTION) {
						...
						if (socket.openConn()) {
							...
							//if (socket.writeReadOneLine("pickabcard c").contains("ok")) {
								...
								if (string.contains("ok")) {
									...
									socket.closeConn();
									finded.setEnabled(true);
									JOptionPane.showMessageDialog(CenterPositon.this,"Socket Receive Failed", "Socket Receive",JOptionPane.WARNING_MESSAGE);
									return;
								}
							}else {
								socket.closeConn();
								finded.setEnabled(true);
								JOptionPane.showMessageDialog(CenterPositon.this,"网络通信异常！", "Socket Receive",JOptionPane.WARNING_MESSAGE);
								return;
							}
						} else {
							socket.closeConn();
							finded.setEnabled(true);
							JOptionPane.showMessageDialog(CenterPositon.this,"网络连接失败！", "Socket Open",JOptionPane.WARNING_MESSAGE);
							return;
						}	
					}
				}).start();
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						if (string.contains("ok")) {
							...
							JOptionPane.showMessageDialog(CenterPositon.this,
		...
		northJPanel.add(xComboBox, new GBC(2, 0).setInsets(5).setWeight(100, 100).setAnchor(GBC.WEST));
	...
	public void addKeyEnvent(JRootPane rp, String keyString, final String moveString) {
		...
		InputMap inputMap = rp.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT);
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					String string1 = xComboBox.getSelectedItem().toString().trim();
					if (string1.length() < 1) {
						JOptionPane.showMessageDialog(CenterPositon.this, "位移输入为空！", "位移量", JOptionPane.WARNING_MESSAGE);
						return;
					}
					try {
						int i = Integer.parseInt(string1);
						if (i > 5 || i < 1) {
							JOptionPane.showMessageDialog(CenterPositon.this, "位移量太大，必须输入1-5！", "位移量", JOptionPane.WARNING_MESSAGE);
							return;
						}
					} catch (NumberFormatException e) {
						e.printStackTrace();
						JOptionPane.showMessageDialog(CenterPositon.this, "位移格式输入 错误,必须是数字！", "位移量", JOptionPane.WARNING_MESSAGE);
						return;
					}
	...
	public static void main(String[] args) {
		CenterPositon centerPositon = new CenterPositon(new JFrame());
